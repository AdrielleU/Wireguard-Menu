...
#                 traffic-log.sh       one line per connection through a tunnel,
#                                      and the rsyslog rule and logrotate policy
#                                      that give both one file and one window
#
...
TRAFFIC_BASE="wireguard-traffic-log"   # a service, no timer: see traffic-log.sh
...
# syslog reserves for custom applications. The traffic log's lines come from the
# kernel instead, labeled "${WG_TRAFFIC_LOG_PREFIX% }" by traffic-log.sh. Both go to one
# file, through ONE action: two actions writing the same file each keep their
# own handle and buffer, which rsyslog advises against. Without this rule the
# records land in /var/log/messages alongside every other service.
#
...
check_retention() {
    ...
    # rsyslog discards silently past its journal intake limit (imjournal:
    # 20000 messages per 10 min, for the whole host) and says so only
    # afterwards, in its own log. A trail with holes has to say so.
    local drops
    drops=$(journalctl -u rsyslog.service --since -30d --no-pager 2>/dev/null | grep -c 'rate-limit')
    if (( drops > 0 )); then
        print_warning "rsyslog dropped messages to rate-limiting ${drops} time(s) in the last 30 days, so the trail is missing lines. See: journalctl -u rsyslog --since -30d | grep rate-limit"
    fi
...
preflight() {
    ...
    local f bad=()
    for f in "$REPO_DIR" "${REPO_DIR}/healthcheck.sh" "${REPO_DIR}/log-connections.sh" \
             "${REPO_DIR}/traffic-log.sh" "${REPO_DIR}/utils.sh"; do
        [[ -e "$f" ]] || continue
...
# The traffic log's share of the preflight: can its ruleset load here, and will
# it see anything once it does.
preflight_traffic() {
    local out
    if ! command -v nft &>/dev/null; then
        pf_fail "nft is not installed, and the traffic log is an nftables table. Install nftables first"
        return
    fi
    # The real ruleset in check mode: whatever start would load, validated by
    # this kernel, with nothing loaded.
    if out=$("${REPO_DIR}/traffic-log.sh" --check 2>&1); then
        print_success "traffic log: its nftables ruleset is valid on this kernel"
    else
        pf_fail "traffic log: nft rejects its ruleset on this kernel: ${out:-no detail}. See: ${REPO_DIR}/traffic-log.sh --dry-run"
    fi
    if [[ "$(sysctl -n net.ipv4.ip_forward 2>/dev/null)" != 1 ]]; then
        print_warning "IP forwarding is off, so no LAN-to-LAN traffic passes through this box: the traffic log will record only connections to the box itself"
    fi
    # `ct state new` needs connection tracking, and loading the ruleset turns it
    # on if nothing had. Once its table is full the kernel drops new connections.
    if [[ ! -d /sys/module/nf_conntrack ]]; then
        print_warning "connection tracking is not in use on this host yet. The traffic log turns it on, and the kernel then drops new connections whenever its table is full (net.netfilter.nf_conntrack_max)"
    fi
}
...
# The traffic log is a service, not a timer: it loads its nftables table once
# and holds it, and the kernel does the logging. restart rather than start, so
# a re-run applies a changed ruleset.
install_traffic() {
    unit_install_service "$REPO_DIR" "$TRAFFIC_BASE"
    if $DRY_RUN; then
        echo "  would run: systemctl daemon-reload; systemctl enable ${TRAFFIC_BASE}.service; systemctl restart ${TRAFFIC_BASE}.service"
        return 0
    fi
    systemctl daemon-reload || die "systemctl daemon-reload failed"
    systemctl enable "${TRAFFIC_BASE}.service" >/dev/null 2>&1 || die "Failed to enable ${TRAFFIC_BASE}.service"
    systemctl restart "${TRAFFIC_BASE}.service" \
        || die "Failed to start ${TRAFFIC_BASE}.service; see: journalctl -u ${TRAFFIC_BASE}.service"
    print_success "${TRAFFIC_BASE}.service enabled and started — traffic goes to ${WG_LOG_FILE} as ${WG_TRAFFIC_LOG_PREFIX% }"
}
...
install_all() {
    ...
    if $DO_LOGGING; then
        echo -e "${CYAN}== audit (connection trail + traffic) ==${NC}"
...
uninstall_all() {
    ...
    $DO_LOGGING     && unit_remove "$TRAFFIC_BASE"   # its ExecStop unloads the table
